...
date:       2020-03-06
author:     YunLambert
header-img: img/post-bg-centos.png
catalog: true
tags:
    - Share
...
事情的起因，是自己的阿里云服务器到期了，最近刚好有一个优惠就放弃了原来的服务器。只是没想到重新配置环境真的要了我的老命......为了以后能减少重新配置的时间以及为想配置相同环境的一些朋友，所以萌生了写一个脚本和步骤的想法，以后可以照本宣科。

## 安装python

参照这篇博文：[https://segmentfault.com/a/1190000009922582](https://segmentfault.com/a/1190000009922582)
...
网速不佳的情况下：`yum install -y python3`

## 安装mySQL

由于mariadb较新且接口和mysql相似，所以直接安装mariadb：
...
安装pip包
通过 pip freeze > requirements.txt 将本地的虚拟环境安装包相信信息导出来,然后将requirements.txt文件上传到服务器之后运行：
```
...
```bash
printf $(tput setaf 4; tput bold)'Auto Linux Configuration Script is running....\n'$(tput sgr0)
# sudo -p "$(tput setaf 6) Enter sudo password$(tput blink):$(tput sgr 0)" whoami 1>/dev/null;
echo "Check environment and install dependencies..."
yum install -y openssl-devel bzip2-devel expat-devel gdbm-devel readline-devel sqlite-devel gcc gcc-c++  openssl-devel libffi-devel python-devel mariadb-devel
...
sudo yum install nginx
sudo systemctl start nginx
yum install python-setuptools python36-devel
pip3 install virtualenvwrapper
export WORKON_HOME=$HOME/.virtualenvs
source /usr/local/bin/virtualenvwrapper.sh
source  ~/.bashrc
